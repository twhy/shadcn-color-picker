    ...
    >
      <div className="flex flex-col items-center justify-center gap-3 bg-gray-50 p-4 rounded-lg shadow-md">
        ...
        <div className="font-mono font-medium flex items-center justify-center">
          RGBA({<ColorValue value={rgba.r} />}, {<ColorValue value={rgba.g} />}, {<ColorValue value={rgba.b} />}, {<ColorValue value={rgba.a.toFixed(2)} />})
        </div>
        <div className="font-mono font-medium flex items-center justify-center">
          HSLA({<ColorValue value={hsla.h} />}, {<ColorValue value={hsla.s} />}, {<ColorValue value={hsla.l} />}, {<ColorValue value={hsla.a.toFixed(2)} />})
        </div>
      </div>
    </main>
  );
...
function ColorValue({ value, size = 3 }: { value: number | string; size?: number }) {
  return (
